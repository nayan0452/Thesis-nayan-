...
struct RDPF : public DPF {
    ...
    inline RegAS unit_as(const LeafNode &leaf) const {
        RegAS a;
        value_t lowword = value_t(_mm_cvtsi128_si64(leaf[0]));
        if (whichhalf == 1) {
            lowword = -lowword;
        }
    ...
    inline RegXSW scaled_xs(const LeafNode &leaf) const {
        ...
        nbits_t j = 0;
        value_t highword =
            value_t(_mm_cvtsi128_si64(_mm_srli_si128(leaf[0],8)));
        x[j++].xshare = highword;
        for (nbits_t i=1;i<LWIDTH;++i) {
            value_t lowword =
                value_t(_mm_cvtsi128_si64(leaf[i]));
            value_t highword =
                value_t(_mm_cvtsi128_si64(_mm_srli_si128(leaf[i],8)));
            x[j++].xshare = lowword;
            if (j < WIDTH) {
                x[j++].xshare = highword;
            }
